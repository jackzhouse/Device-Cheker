...
const latestCheck = await DeviceCheck.findOne({ employeeId }).sort({
  version: -1,
});
...
```typescript
const checks = await DeviceCheck.find(filters)
...
export async function deleteEmployee(id: string): Promise<APIResponse<any>> {
  const response = await fetch(`/api/employees/${id}`, {
    method: "DELETE",
  });
