...
import { startUserSyncJob } from '@/lib/auth/sync-job';
  ...
  try {
    ...
    if (!credentialToken) {
      return NextResponse.json(
        { success: false, error: 'Token Katalis tidak tersedia. Silakan login ulang.' },
        { status: 401 }
      );
    }
    ...
    const job = startUserSyncJob(credentialToken, size);
